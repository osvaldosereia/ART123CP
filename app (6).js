...
function mustUseEmbedded(){ return false; }
async async function fetchText(path){
  try{ const res = await fetch(path, {cache:'no-store'}); if(res.ok) return await res.text(); }catch(e){ console.warn('fetchText failed', path, e); }
  return '';
...
async function loadJSON(path, fallback=undefined){
  try{ const res=await fetch(path,{cache:'no-store'}); if(res.ok) return await res.json(); }catch(e){ console.warn('loadJSON failed', path, e); }
  const v = (fallback!==undefined) ? fallback : null;
  if(v==null) console.error('JSON não encontrado:', path);
  return v;
...
async function loadTxtAsQuiz(path){
  ...
  if(!questions || questions.length===0){ toast('Nenhuma questão em: '+path, 'error', 3000); }
  const parts = path.split('/');
  const category = parts[1] || 'Geral';
  const file = (parts[parts.length-1]||'Quiz.txt').replace(/\.txt$/i,'');
  return { meta: { category, title: file }, questions };
}
...
async async function loadQuiz(path, fresh=false, tryRestore=false){
...
function renderTagBar(currentList=null){
  ...
  if(!el) return;
  el.innerHTML = '';
  const qcur = q; // usa questão atual
  if(!qcur || !Array.isArray(qcur.tags) || qcur.tags.length===0) return;
  el.innerHTML = qcur.tags.map(t => `<a href="#tag:${encodeURIComponent(t)}" data-tag="${t}" style="text-decoration: underline; white-space: nowrap; margin-right:8px;">${htmlEscape(t)}</a>`).join(', ').replace(/,\s*$/,'');
