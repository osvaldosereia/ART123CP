...
export function renderFicha(el, artigo, extras, openFerramentas, onRelatedSearch, onOpenCodigo, openRemissoes){
  el.innerHTML='';
  ...
  const totalRem = sumRemissoes(artigo);
  const footer = document.createElement('div'); footer.className='footer-chips';
  if(totalRem>0){
    const chipR = document.createElement('button'); chipR.className='footer-chip'; chipR.textContent=`Remissões (${totalRem})`;
    chipR.addEventListener('click', ()=> openRemissoes(artigo));
    footer.appendChild(chipR);
  }
  const chipF = document.createElement('button'); chipF.className='footer-chip'; chipF.textContent='Ferramentas';
  chipF.addEventListener('click', (ev)=> openFerramentas(ev.currentTarget, artigo));
  footer.appendChild(chipF);
