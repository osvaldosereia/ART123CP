const CONFIG = {
    ...
    // PARSER TXT atualizado: aceita "-(A)" e anuladas
    function parseTxtQuestions(raw){
      ...
      for(const blockRaw of blocks){
        const block = blockRaw.replace(/\[cite_start\]/gi,'').replace(/\[cite:[^\]]*\]/gi,'');
        ...
        const optARegex = /^\s*-\s*\(?A\)\s*/m;
        const aIdx = block.search(optARegex);
        const hIdx = block.indexOf('#');
        let head = '';
        if(hIdx >= 0 && (aIdx === -1 || aIdx > hIdx)){
          head = block.slice(hIdx).replace(/^#\s*/,'').slice(0, aIdx > -1 ? (aIdx - hIdx) : undefined).trim();
        }
        ...
        if(!head || options.length===0) continue;
