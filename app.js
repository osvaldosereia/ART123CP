...
function parseTxtQuestions(raw){
  ...
  for(const blockRaw of blocks){
    ...
    // coleta linhas finais iniciadas por '#'
    const lines = block.split('\n');
    let tags = [];
    while(lines.length && lines[lines.length-1].trim().startsWith('#')){
      const line = lines.pop();
      tags = [...new Set([...(parseTagLine(line)), ...tags])];
    }
    block = lines.join('\n');
...
function renderTagBar(currentList=null){
  const pool = currentList || getVisibleQuestions();
  ...
  if(sorted.length===0){
    const s = document.createElement('span'); s.className='muted'; s.textContent='sem tags'; tagsBarEl.appendChild(s); return;
  }
  for(const [t,c] of sorted){
    const a = tagChip(t);
    a.title = `${c} ocorrência(s)`;
    if(TAG_FILTER === t) a.classList.add('active');
    tagsBarEl.appendChild(a);
  }
}
